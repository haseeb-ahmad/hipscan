...
xml.countrydata do
  ...
  for country in @countries.map{|a| {:name => a, :count => @scans.count{|s| s.country == a } } }.sort{|a,b| b[:count] <=> a[:count]}
    unless country[:name].blank?
      xml.state :id => country[:name] do
        xml.name country[:name]
        xml.data '0'
        xml.hover "This country has #{country[:count]} scans."
      end
    end
  end
